...
/**
 * La clase Elfo hereda de la clase Personaje. Completa ciertos atributos que estaban
 * declarados en la clase Personaje, como por ejemplo habilidadesRaza[]
 */
public class Elfo extends Personaje {
	 ...
	 * se retornará false El método serAtacado() posee como argumento la suma de la
	 * fuerza del atacante y el nivel del mismo multiplicado por 10
	 * @param atacado
	 *            Instancia de Personaje o de NPC, dependiendo de cual sea,
	 *            será como responda el método serAtacado()
	...
	public final boolean habilidadRaza1(final Peleable atacado) {
		...
		return tieneEnergia && (atacado.serAtacado(this.getFuerza()
				+ this.getNivel() * MULTIPLICADORNIVEL) > 0);
	}
	 ...
	 * atacante(caster) posea 10 o más del atributo energia ya que estos se descuentan
	 * seguido de comprobar que los posee, de lo contrario el ataque no será posible y
	 * se retornará false El método serAtacado() posee como argumento un entero
	 * representando 1 o 0, dependiendo si el atributo magia del llamador, es mayor a 0
	 * @param atacado
	 *            Instancia de Personaje o de NPC, dependiendo de cual sea,
	 *            será como responda el método serAtacado()
	 * @return retorna si se ejecutó correctamente la habilidad
	...
	public final boolean habilidadRaza2(final Peleable atacado) {
		...
		if (this.getEnergia() >= ENERGIAMINIMA) {
			...
			return tieneEnergia && (atacado.serAtacado((this.getMagia())) > 0);
	}
